...
module Jekyll
  ...
  class FlickrPhotoURLTag < Liquid::Tag
    ...
    def render(context)
        @photo = { :id => lookup(context, @params[0]), :size => @params[1] || "Medium", :sizes => {}, :title => "", :caption => "", :url => "", :exif => {} }
        @api_key = context.registers[:site].config["flickr"]["api_key"]
        @photo.merge!(@@cached[photo_key] || get_photo)

        selected_size = @photo[:sizes][@params[1] || "Medium"]
...
Liquid::Template.register_tag('flickr_photo_url', Jekyll::FlickrPhotoURLTag)
